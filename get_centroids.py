...
# 1.1) Leer el dataframe de estaciones
stations_df = pd.read_csv(f'{source_data_directory}/stations.csv')
df_yield = pd.read_csv(f'{source_data_directory}/crop_yield.csv')
...
# 1) Reproyectar el GeoDataFrame principal (que todavía contiene polígonos, no centroides)
counties_cornbelt_wgs84 = counties_cornbelt_proj.to_crs(epsg=4326)

# 2) Calcular centroides con la nueva geometría en EPSG:4326
...
# 2.1 Crear la columna STATEFP a partir de state_alpha
df_yield["STATEFP"] = df_yield["state_alpha"].map(state_alpha_to_fips)
df_yield["COUNTYFP"] = df_yield["county_code"].astype(str).str.zfill(3)
print(df_yield.head())
print(counties_cornbelt_wgs84.columns)
counties_cornbelt_wgs84["COUNTYFP"] = counties_cornbelt_wgs84["COUNTYFP"].astype(str).str.zfill(3)

df_merged = df_yield.merge(
    counties_cornbelt_wgs84[["STATEFP", "COUNTYFP", "lat_centroid", "lon_centroid"]],
    on=["STATEFP", "COUNTYFP"],
    how="left"
)

df_merged.to_csv(f'{source_data_directory}/counties_centroids.csv')
df_clean = df_merged.dropna(subset=["lat_centroid", "lon_centroid"])
print(df_merged["lat_centroid"].isna().sum())
df_clean.to_csv(f"{source_data_directory}/counties_centroids_cornbelt.csv")
...
os.makedirs('shape_files/counties_usda', exist_ok=True)
joined_single_geom.to_file("shape_files/counties_usda/counties_nearest_usda_station.shp")

# Para CSV, debes quitar la geometría (por ejemplo joined.drop(columns="geometry"))
joined.drop(columns="geometry").to_csv(f"{source_data_directory}/counties_nearest_usda_station.csv", index=False)

print("Listo. Cada condado se asignó a la estación más cercana vía Voronoi.")